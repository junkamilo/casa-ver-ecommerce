  ...
  return (
    <div className="group relative flex flex-col w-[65vw] sm:w-[40vw] md:w-[calc(25%-18px)] shrink-0 bg-white border border-gray-200/80 shadow-[0_2px_12px_-4px_rgba(0,0,0,0.04)] hover:shadow-[0_20px_40px_-15px_rgba(21,71,52,0.15)] hover:-translate-y-1.5 transition-all duration-500 overflow-hidden cursor-pointer">
      <Link href={`/product/${item.slug}`} className="relative aspect-[3/4] overflow-hidden bg-[#F8F9FA] block m-2">
        ...
        {item.badge && (
          <div className={`absolute top-3 left-3 text-[9px] font-black px-3 py-1.5 uppercase tracking-[0.2em] shadow-md z-10 transition-colors ${badgeClass}`}>
            {item.badge}
          </div>
        )}
      ...
      <div className="px-4 pb-5 pt-3 flex flex-col gap-2.5 bg-white">
        ...
        <div className="flex flex-col gap-3 mt-1">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm font-medium text-[#154734]" style={{ fontFamily: "Georgia, serif" }}>
              {item.price}
            </span>
            {item.oldPrice && (
              <span className="text-xs text-gray-400 line-through decoration-gray-300 decoration-1" style={{ fontFamily: "Georgia, serif" }}>
                {item.oldPrice}
              </span>
            )}
          </div>
