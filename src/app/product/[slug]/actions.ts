...
export async function rateProduct(
  ...
  if (rating < 1 || rating > 5) return { success: false, error: "Calificación inválida" };
  return saveReview(productId, slug, { rating });
}
...
export async function getUserReview(productId: string) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return (prisma.review as any).findUnique({
    where: { userId_productId: { userId, productId } },
    select: { rating: true, comment: true },
  }) as Promise<{ rating: number; comment: string | null } | null>;
}
