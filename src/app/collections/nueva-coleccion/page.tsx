...
async function fetchNewCollection(): Promise<{
  ...
  const products: CollectionProduct[] = raw.map((p) => {
    ...
    return {
      ...
      badge: p.comparePrice ? "Oferta" : p.isFeatured ? "Destacado" : undefined,
      colors:
        p.colors.length > 0
...
export default async function NuevaColeccionPage() {
  const { products, filterOptions } = await fetchNewCollection();
